...
public:
    ...
    virtual Result<Bytes> ecdhCompute(const EcPrivateKey& priv,
                                      const EcPublicKey& peerPub) = 0;
    ...
    virtual Result<bool> verify(ByteView data, const Signature& sig,
                                const EcPublicKey& key) = 0;

    /// AES-128-GCM encrypt. Returns ciphertext || 16-byte tag.
    virtual Result<Bytes> aesGcmEncrypt(ByteView plaintext, const SessionKey& key,
                                        ByteView nonce, ByteView aad) = 0;

    /// AES-128-GCM decrypt and verify tag. Returns plaintext.
    virtual Result<Bytes> aesGcmDecrypt(ByteView ciphertext, const SessionKey& key,
                                        ByteView nonce, ByteView aad) = 0;

    /// HKDF-SHA-256 key derivation.
    virtual Result<Bytes> hkdfDerive(ByteView ikm, ByteView salt,
                                     ByteView info, size_t outputLen) = 0;
