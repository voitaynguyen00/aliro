...
MbedTlsCryptoProvider::MbedTlsCryptoProvider() {
    ...
    mInitialized = (mbedtls_ctr_drbg_seed(&mCtrDrbg, mbedtls_entropy_func, &mEntropy,
                                           kPers, sizeof(kPers) - 1) == 0);
}
...
Result<EcKeyPair> MbedTlsCryptoProvider::generateKeyPair() {
    if (!mInitialized) return tl::unexpected(AliroError::CRYPTO_FAILURE);

    MbedEcpKp kp;
    if (mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_SECP256R1, kp.get(),
    ...
    size_t olen = 0;
    if (mbedtls_ecp_point_write_binary(grp.get(), Q.get(),
                                        MBEDTLS_ECP_PF_UNCOMPRESSED,
                                        &olen, pubBytes.data(), kP256PubSize) != 0
        || olen != kP256PubSize)
        return tl::unexpected(AliroError::CRYPTO_FAILURE);
...
Result<Bytes> MbedTlsCryptoProvider::ecdhCompute(const EcPrivateKey& priv,
    ...
    MbedEcpGroup grp;
    if (loadP256Group(*grp.get()) != 0) return tl::unexpected(AliroError::CRYPTO_FAILURE);
    ...
    MbedEcpPoint Q;
    if (mbedtls_ecp_point_read_binary(grp.get(), Q.get(),
                                       peerPub.data.data(), kP256PubSize) != 0)
        return tl::unexpected(AliroError::CRYPTO_FAILURE);

    MbedEcpPoint shared;
    if (mbedtls_ecp_mul(grp.get(), shared.get(), d.get(), Q.get(),
                         mbedtls_ctr_drbg_random, &mCtrDrbg) != 0)
        return tl::unexpected(AliroError::CRYPTO_FAILURE);
...
Result<Signature> MbedTlsCryptoProvider::sign(ByteView data, const EcPrivateKey& key) {
    ...
    MbedMpi r, s;
    if (mbedtls_ecdsa_sign(grp.get(), r.get(), s.get(), d.get(),
                            hash.data(), hash.size(),
                            mbedtls_ctr_drbg_random, &mCtrDrbg) != 0)
        return tl::unexpected(AliroError::CRYPTO_FAILURE);
...
Result<bool> MbedTlsCryptoProvider::verify(ByteView data, const Signature& sig,
    ...
    int ret = mbedtls_ecdsa_verify(grp.get(), hash.data(), hash.size(),
                                    Q.get(), r.get(), s.get());
    if (ret == 0)                              return true;
    if (ret == MBEDTLS_ERR_ECP_VERIFY_FAILED) return false;
    return tl::unexpected(AliroError::CRYPTO_FAILURE);
}
...
// aesGcmEncrypt
// ---------------------------------------------------------------------------
Result<Bytes> MbedTlsCryptoProvider::aesGcmEncrypt(ByteView      plaintext,
    ...
    MbedGcm ctx;
    if (mbedtls_gcm_setkey(ctx.get(), MBEDTLS_CIPHER_ID_AES,
                            key.data.data(),
    ...
    if (mbedtls_gcm_crypt_and_tag(ctx.get(), MBEDTLS_GCM_ENCRYPT,
                                   plaintext.size(),
...
// aesGcmDecrypt
// ---------------------------------------------------------------------------
Result<Bytes> MbedTlsCryptoProvider::aesGcmDecrypt(ByteView      ciphertext,
                                                     const SessionKey& key,
                                                     ByteView      nonce,
                                                     ByteView      aad) {
    ...
    int ret = mbedtls_gcm_auth_decrypt(ctx.get(), dataLen,
                                        nonce.data(), nonce.size(),
                                        aad.empty() ? nullptr : aad.data(), aad.size(),
                                        tagPtr, 16,
                                        dataLen == 0 ? nullptr : ciphertext.data(),
                                        dataLen == 0 ? nullptr : plaintext.data());
    if (ret != 0) return tl::unexpected(AliroError::CRYPTO_FAILURE);
    return plaintext;
}
...
// hkdfDerive
// ---------------------------------------------------------------------------
Result<Bytes> MbedTlsCryptoProvider::hkdfDerive(ByteView ikm, ByteView salt,
                                                  ByteView info, size_t outputLen) {
    const mbedtls_md_info_t* md = mbedtls_md_info_from_type(MBEDTLS_MD_SHA256);
    if (!md) return tl::unexpected(AliroError::CRYPTO_FAILURE);
