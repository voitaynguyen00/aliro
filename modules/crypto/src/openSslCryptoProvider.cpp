...
struct PkeyDel     { void operator()(EVP_PKEY* p)       const noexcept { EVP_PKEY_free(p); } };
struct PkeyCtxDel  { void operator()(EVP_PKEY_CTX* p)   const noexcept { EVP_PKEY_CTX_free(p); } };
struct MdCtxDel    { void operator()(EVP_MD_CTX* p)     const noexcept { EVP_MD_CTX_free(p); } };
struct CipherDel   { void operator()(EVP_CIPHER_CTX* p) const noexcept { EVP_CIPHER_CTX_free(p); } };
struct KdfDel      { void operator()(EVP_KDF* p)        const noexcept { EVP_KDF_free(p); } };
struct KdfCtxDel   { void operator()(EVP_KDF_CTX* p)    const noexcept { EVP_KDF_CTX_free(p); } };
struct EcSigDel    { void operator()(ECDSA_SIG* p)      const noexcept { ECDSA_SIG_free(p); } };
struct BnDel       { void operator()(BIGNUM* p)         const noexcept { BN_free(p); } };
struct ParamBldDel { void operator()(OSSL_PARAM_BLD* p) const noexcept { OSSL_PARAM_BLD_free(p); } };
struct ParamDel    { void operator()(OSSL_PARAM* p)     const noexcept { OSSL_PARAM_free(p); } };
...
PkeyPtr evpKeyFromPrivate(const EcPrivateKey& privKey) {
    ...
    if (!ctx || EVP_PKEY_fromdata_init(ctx.get()) <= 0) return nullptr;

    EVP_PKEY* pkey = nullptr;
    if (EVP_PKEY_fromdata(ctx.get(), &pkey, EVP_PKEY_KEYPAIR, params.get()) <= 0) return nullptr;
    return PkeyPtr(pkey);
}
...
PkeyPtr evpKeyFromPublic(const EcPublicKey& pubKey) {
    ...
    EVP_PKEY* pkey = nullptr;
    if (EVP_PKEY_fromdata(ctx.get(), &pkey, EVP_PKEY_PUBLIC_KEY, params.get()) <= 0) return nullptr;
    return PkeyPtr(pkey);
}
...
Result<EcKeyPair> OpenSslCryptoProvider::generateKeyPair() {
    ...
    OSSL_PARAM params[] = {
        OSSL_PARAM_utf8_string("group", const_cast<char*>("P-256"), 5),
        OSSL_PARAM_END
    };
...
Result<Signature> OpenSslCryptoProvider::sign(ByteView data, const EcPrivateKey& key) {
    ...
    if (!pkey) return tl::unexpected(AliroError::CRYPTO_FAILURE);
    ...
    if (!ecSig) return tl::unexpected(AliroError::CRYPTO_FAILURE);
...
Result<bool> OpenSslCryptoProvider::verify(ByteView data, const Signature& sig,
    ...
    if (sig.data.size() != 64) return tl::unexpected(AliroError::INVALID_MESSAGE);
    ...
    if (!r || !s) return tl::unexpected(AliroError::CRYPTO_FAILURE);
    ECDSA_SIG_set0(ecSig.get(), r.release(), s.release());

    int derLen = i2d_ECDSA_SIG(ecSig.get(), nullptr);
    if (derLen <= 0) return tl::unexpected(AliroError::CRYPTO_FAILURE);
    ...
    int rc = EVP_DigestVerify(ctx.get(), derSig.data(), derSig.size(),
                               data.data(), data.size());
    if (rc == 1) return true;
...
// aesGcmEncrypt
// ---------------------------------------------------------------------------
Result<Bytes> OpenSslCryptoProvider::aesGcmEncrypt(ByteView plaintext,
    ...
    if (!ctx) return tl::unexpected(AliroError::CRYPTO_FAILURE);
...
// aesGcmDecrypt
// ---------------------------------------------------------------------------
Result<Bytes> OpenSslCryptoProvider::aesGcmDecrypt(ByteView ciphertext,
                                                    const SessionKey& key,
                                                    ByteView nonce, ByteView aad) {
    ...
    if (dataLen > 0) {
        if (EVP_DecryptUpdate(ctx.get(), plaintext.data(), &outLen,
                               ciphertext.data(), static_cast<int>(dataLen)) <= 0)
            return tl::unexpected(AliroError::CRYPTO_FAILURE);
    }

    if (EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_SET_TAG, 16,
                              const_cast<uint8_t*>(tagPtr)) <= 0)
        return tl::unexpected(AliroError::CRYPTO_FAILURE);
...
// hkdfDerive
// ---------------------------------------------------------------------------
Result<Bytes> OpenSslCryptoProvider::hkdfDerive(ByteView ikm, ByteView salt,
                                                  ByteView info, size_t outputLen) {
    KdfPtr kdf(EVP_KDF_fetch(nullptr, "HKDF", nullptr));
    if (!kdf) return tl::unexpected(AliroError::CRYPTO_FAILURE);
    ...
    params.push_back(OSSL_PARAM_construct_utf8_string(
        OSSL_KDF_PARAM_DIGEST, const_cast<char*>("SHA256"), 6));
