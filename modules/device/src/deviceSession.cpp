...
Result<Bytes> deriveSessionKey(ICryptoProvider& crypto, const EcPrivateKey& deviceEphPriv,
                               const EcPublicKey& readerEphPub, ByteView readerNonce,
                               ByteView deviceNonce) {
...
DeviceSession::DeviceSession(ICryptoProvider& crypto, const EcPrivateKey& devicePrivKey,
...
Result<Bytes> DeviceSession::handleAuth0(ByteView commandData) {
    ...
    for (auto& item : *items) {
        if (item.tag == protocol::kTagEphemeralPublicKey)
            readerEphPubBytes = item.value;
        else if (item.tag == protocol::kTagReaderNonce)
            readerNonce = item.value;
        else if (item.tag == protocol::kTagReaderIdentifier)
            readerLongTermPubBytes = item.value;
    }
    ...
    if (!kp)
        return tl::unexpected(kp.error());
    mDeviceEphemeralKp = *kp;
    mDeviceEphemeralPub = kp->pub;

    auto deviceNonceResult = mCrypto.randomBytes(16);
    if (!deviceNonceResult)
        return tl::unexpected(deviceNonceResult.error());
    mDeviceNonce = std::move(*deviceNonceResult);

    mTranscript = makeTranscript(readerEphPubBytes, readerNonce, kp->pub.data, mDeviceNonce);

    auto sig = mCrypto.sign(mTranscript, mDevicePrivKey);
    if (!sig)
        return tl::unexpected(sig.error());

    auto sk = deriveSessionKey(mCrypto, kp->priv, mReaderEphemeralPub, readerNonce, mDeviceNonce);
    if (!sk)
        return tl::unexpected(sk.error());
...
Result<Bytes> DeviceSession::handleAuth1(ByteView commandData) {
    ...
    for (auto& item : *items) {
        if (item.tag == protocol::kTagSignature)
            readerSigBytes = item.value;
    }
