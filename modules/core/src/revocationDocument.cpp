...
Result<Bytes> encodeRevocationDataElement(const RevocationDataElement& elem) {
    cbor::Encoder enc;
    size_t fieldCount = 2 + (elem.entries.empty() ? 0 : 1) + (elem.entriesRemove.empty() ? 0 : 1);
    enc.beginMap(fieldCount);

    enc.addUint(0);
    enc.addUint(elem.version);
    enc.addUint(2);
    enc.addUint(static_cast<uint64_t>(elem.changeMode));

    if (!elem.entries.empty()) {
        enc.addUint(3);
        enc.beginArray(elem.entries.size());
        for (const auto& e : elem.entries)
            enc.addBytes(e.credentialId);
    }
    if (!elem.entriesRemove.empty()) {
        enc.addUint(4);
        enc.beginArray(elem.entriesRemove.size());
        for (const auto& e : elem.entriesRemove)
            enc.addBytes(e.credentialId);
    }
    return enc.finish();
}
...
Result<Bytes> encodeRevocationDocument(const RevocationDocument& doc) {
    ...
    if (!issuerAuthBytes)
        return tl::unexpected(issuerAuthBytes.error());
    ...
    for (const auto& elem : doc.revocationElements) {
        ...
        if (!elemBytes)
            return tl::unexpected(elemBytes.error());
